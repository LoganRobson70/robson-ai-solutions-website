...
- The corrected maturity model, flagship-gallery hierarchy and approved brand-contrast treatment are committed, pushed and validated on a refreshed non-production preview.
- The live Building Analyst app privacy notice is reconciled into the normal website build and first-class release QA so future releases preserve it.
- Review the validated preview and stop at the separate production deployment approval gate.
...
- The final exact-candidate local rerun passed all 41 stages immediately before commit. Artifact: `output/release-local-gate/gate-2026-07-17T09-18-44-104Z/release-local-gate.json`; measurement pack: `output/measurement/evidence-2026-07-17T09-21-12-886Z`.
- Exactly the thirteen manifest files were committed as `35846ce` (`Preserve Building Analyst app privacy route`) and pushed normally to `origin/codex/brand-contrast-system-20260716`; local and remote hashes matched before the preview deploy.
- Refreshed Netlify non-production deploy `6a59f4688bee702f1193d9ea` is ready at `https://product-maturity-privacy-0717--robson-ai-website.netlify.app`; logs: `https://app.netlify.com/projects/robson-ai-website/deploys/6a59f4688bee702f1193d9ea`.
- The complete 14-stage deployed-preview gate passed against immutable URL `https://6a59f4688bee702f1193d9ea--robson-ai-website.netlify.app`. Artifact: `output/release-preview-gate/gate-2026-07-17T09-23-02-533Z/release-preview-gate.json`. The app privacy route returned HTTP 200; deployed security, headers, protected-path denies, BuildScan, keyboard, semantic/SEO, product/design, 42 responsive, 18 visual-polish, rendered and measurement checks passed.
- Final in-app Browser review at 1280 x 720 confirmed the corrected product-status wording, `#product-visuals` immediately after the Building Analyst hero, all five gallery image elements decoded, the approved shadow composite rendering as a single visible brand mark on each applicable dark surface, no horizontal overflow, the app privacy title and 17 July 2026 effective date, and no warning/error console messages.
- Preview readiness is 100% and production readiness is 100% from a candidate-validation perspective. Production remains unchanged at deploy `6a59edaeb162561cffa6210c`; publishing commit `35846ce` still requires Wayne's separate explicit production approval.

Background production preflight completed while Wayne was away on 2026-07-17:

- No production deploy, `main` push, merge, DNS/domain change, external message, commit, tag, rollback or destructive action was performed.
- A fresh `npm run build:site` left the release source unchanged. All 51 files in `dist/` matched immutable preview deploy `6a59f4688bee702f1193d9ea` byte-for-byte by SHA-256 and length, with zero mismatches.
- Compared with current production, 43 of 51 built public files are byte-identical. The eight expected public deltas are `index.html`, `building-analyst.html`, `building-analyst-privacy.html`, `who-its-for.html`, `privacy.html`, `404.html`, `sitemap.xml` and `styles.css`. These cover the truthful maturity wording, flagship-gallery placement, first-class app privacy route, aligned shared labels, sitemap entry and CSP-compatible privacy layout rule. All marketing images, scripts, fonts, BuildScan assets and vendor files remain identical.
- Netlify still reports current production deploy `6a59edaeb162561cffa6210c` as ready and candidate branch deploy `6a59f4688bee702f1193d9ea` as ready. Current production is the one-off app privacy deploy and has no Git commit reference; the candidate preview title identifies commit `35846ce`.
- A read-only `QA_PRODUCTION_URL=https://robsonai.co.uk CONFIRM_PRODUCTION_VERIFICATION=true npm run qa:release:production` baseline check stopped at deployed headers because live `/holding.html` returns HTTP 200 instead of the required 301. Artifact: `output/release-production-gate/gate-2026-07-17T09-34-51-191Z/release-preview-gate.json`. No production state changed.
- Follow-up read-only checks proved the live BuildScan viewer, keyboard journeys and semantic/SEO smoke still pass. Product/design acceptance then stopped on the known stale live maturity wording (`BuildScan is working product proof`), so the current production deploy is an emergency rollback state with documented drift, not a clean gate-passing release baseline.
- GitHub `main` remains `52e6253`; remote release branch and local committed HEAD both remain `35846ce`. `main` is an ancestor exactly two commits behind the candidate, so the lowest-risk production path is a normal no-force fast-forward of `main` to exact commit `35846ce`, allowing the configured Netlify Git deployment to align source control and production in one action.
- Read-only remote preflight `git push --dry-run --porcelain origin 35846ce:main` succeeded and reported the expected `52e6253..35846ce` fast-forward. Netlify site metadata still links the GitHub repository with production branch `main`; `netlify.toml` remains the source-controlled build/publish configuration. The dry run changed neither GitHub nor Netlify state.
- Required production approval phrase: `Approve a normal no-force fast-forward of main from 52e6253 to 35846ce, the resulting Netlify production deployment to robsonai.co.uk, full post-deploy verification, and restoration of deploy 6a59edaeb162561cffa6210c if that verification fails.` Before execution, re-fetch `main` and stop if it no longer equals `52e6253`.
- Rollback order after an approved deployment: first restore current deploy `6a59edaeb162561cffa6210c` to recover today's live privacy route and public site, accepting its documented holding-redirect and maturity-copy drift; then prepare a normal revert commit rather than force-pushing. Previous Git-backed production deploy `6a594d18110d2b0008fa427b` at `52e6253` is a secondary emergency fallback only because it predates the app privacy route.
- After successful production verification, a separate small approval will still be needed to commit and push the final tracker/production evidence without bundling unrelated work.

Production approval preflight interruption and privacy cross-link reconciliation on 2026-07-17:

- Wayne supplied the exact production approval phrase for commit `35846ce` and rollback deploy `6a59edaeb162561cffa6210c`. Before any Git write, the mandatory re-fetch confirmed GitHub `main` was still `52e6253` and the candidate branch was still `35846ce`, but Netlify production had changed concurrently to deploy `6a5a39988351f4bd95f27bf9` (`Publish Building Analyst privacy cross-link 20260717`). No `main` push, merge, production deploy or rollback was performed.
- A 51-file comparison between the approved rollback deploy and new production showed exactly one changed public file: `privacy.html`. The new live file added a useful `/building-analyst-privacy` cross-link but also replaced the current privacy page with an older June shell, old navigation, `styles.css?v=20260627a`, `script.js?v=20260627a` and materially reduced current privacy detail.
- The safe local reconciliation preserves the validated current privacy page and adds only the legitimate cross-link as a first article, with the visible update date advanced to 17 July 2026. `scripts/product-design-acceptance-smoke.mjs` now requires a direct, clearly named link from website privacy to the Building Analyst app privacy notice.
- The exact local-only reconciliation is four tracked files: `privacy.html`, `scripts/product-design-acceptance-smoke.mjs`, `docs/codex/RELEASE_STAGING_MANIFEST.md` and `docs/codex/TRACKER.md`, all based on commit `35846ce`. No staging, commit, push, preview deploy or production deploy is approved for this new candidate.
- After correcting the new cross-link to use the existing accessible button treatment, the exact four-file candidate passed the complete 41-step local release gate. Artifact: `output/release-local-gate/gate-2026-07-17T16-15-51-648Z/release-local-gate.json`; evidence pack: `output/measurement/evidence-2026-07-17T16-19-01-137Z`. Evidence records zero axe violations across seven routes, 48 responsive checks passed, product/design acceptance passed and Lighthouse scored 96 performance with 100 accessibility, best practices and SEO, CLS 0 and median LCP about 2.41 seconds. Release inventory found zero secret findings and zero external GLB URIs; production dependencies have zero vulnerabilities. Existing local Firefox/WebKit unavailability and 17 moderate development/release-tooling findings remain warning-only.
- The earlier production approval cannot safely execute unchanged because its rollback deploy is no longer current and the release commit must change to preserve the legitimate live-only cross-link. The next gate is targeted and complete local validation, followed by a new exact commit/preview approval.

Privacy cross-link preview release approved and completed on 2026-07-17:

- Wayne explicitly approved exact-path staging of the four reconciliation files, a scoped commit and normal branch push, a fresh non-production Netlify preview and full deployed-preview verification. Production remained separately approval-gated.
- The final exact snapshot passed the complete 41-step local release gate immediately before release. Artifact: `output/release-local-gate/gate-2026-07-17T16-27-20-241Z/release-local-gate.json`; evidence pack: `output/measurement/evidence-2026-07-17T16-30-25-494Z`.
- Scoped commit `34a847f` (`Preserve Building Analyst privacy cross-link`) contains exactly the four approved files and was pushed normally to `origin/codex/brand-contrast-system-20260716`. Local and remote branch hashes match at `34a847fe668219fb834d64c7369cd469176a6605`; GitHub `main` remains unchanged at `52e6253`.
- Fresh non-production deploy `6a5a5ded7b3bd54f888a3a57` is ready at `https://privacy-crosslink-34a847f--robson-ai-website.netlify.app`; immutable proof URL: `https://6a5a5ded7b3bd54f888a3a57--robson-ai-website.netlify.app`; logs: `https://app.netlify.com/projects/robson-ai-website/deploys/6a5a5ded7b3bd54f888a3a57`.
- The complete 14-step deployed-preview gate passed against the immutable URL. Artifact: `output/release-preview-gate/gate-2026-07-17T16-53-25-890Z/release-preview-gate.json`. It recorded a clean release inventory, zero secret findings, zero external GLB URIs, zero production dependency vulnerabilities, deployed headers/source-path denies, BuildScan, keyboard, semantic/SEO, product/design, 42 responsive, 18 visual-polish, rendered and measurement passes. Existing 17 moderate development/release-tooling findings and unavailable local Firefox/WebKit binaries remain warning-only; Chromium passed.
- Final in-app Browser verification confirmed `#product-visuals` is the first section after the hero, all five gallery images decode, the three marketing compositions remain visible with the approved brand treatment, no horizontal overflow exists and no warning/error console messages were recorded. The `/privacy` page shows the complete current website privacy notice dated 17 July 2026; its single clearly named app-privacy link opened `/building-analyst-privacy`, whose on-device processing and Professional-subscription wording rendered without overflow. The validated friendly preview was left open for Wayne.
- Final read-only Netlify verification confirmed production is still unchanged at ready deploy `6a5a39988351f4bd95f27bf9` (`Publish Building Analyst privacy cross-link 20260717`). No production deploy, rollback, `main` update or force action occurred. A new production approval must name commit `34a847f`, the then-current `main` precondition and the then-current production rollback deploy after a fresh preflight.
- Final release preflight confirmed `origin/main` at `52e6253` is an ancestor three commits behind `34a847f`. Read-only `git push --dry-run --porcelain origin 34a847f:main` reported the expected normal `52e6253..34a847f` fast-forward and changed no remote state. Required production approval phrase: `Approve a normal no-force fast-forward of main from 52e6253 to 34a847f, the resulting Netlify production deployment to robsonai.co.uk, full post-deploy verification, and restoration of deploy 6a5a39988351f4bd95f27bf9 if that verification fails.` Re-fetch both GitHub and Netlify immediately before execution and stop if either precondition has changed.

Production release approved and completed on 2026-07-17:

- Wayne approved the exact normal no-force `main` fast-forward from `52e6253` to `34a847f`, the resulting Netlify production deployment, full post-deploy verification and restoration of deploy `6a5a39988351f4bd95f27bf9` if verification failed. Immediate preflight reconfirmed every named GitHub and Netlify precondition before mutation.
- GitHub `main` fast-forwarded normally to `34a847fe668219fb834d64c7369cd469176a6605`; no merge commit, force-push or unrelated working-tree content was included. `origin/main`, the local committed `HEAD` and `origin/codex/brand-contrast-system-20260716` now match exactly.
- Netlify production deploy `6a5a61526773ce00086c4c1f` (`Preserve Building Analyst privacy cross-link`) is ready, current and tied to commit `34a847f` on branch `main`; published at `2026-07-17T17:07:43.225Z` to `https://robsonai.co.uk`.
- The complete 14-step production release gate passed. Artifact: `output/release-production-gate/gate-2026-07-17T17-08-16-122Z/release-preview-gate.json`. Evidence includes deployed security headers and source-path denies, the interactive BuildScan viewer, keyboard journeys, semantic/SEO, product/design acceptance, 42 responsive checks, 18 visual-polish checks, rendered desktop/mobile screenshots and 23 successful route/asset checks. Production dependencies remain at zero vulnerabilities and the release inventory found zero secret findings and zero external GLB URIs. Existing 17 moderate development/release-tooling findings and unavailable local Firefox/WebKit binaries remain warning-only; Chromium passed.
- Final live Browser verification confirmed the corrected homepage maturity wording, Building Analyst as the flagship product in development, BuildScan in development, neither product released, Property Operations as roadmap exploration and the removed `AI proof property` phrase absent. `#product-visuals` remains the first section after the Building Analyst hero, all five gallery images decode, the approved brand treatment is visible, no horizontal overflow exists and no warning/error console messages were recorded.
- The live `/privacy` page retains the complete current website notice dated 17 July 2026 and its single clearly named link opened `/building-analyst-privacy`; the app notice rendered its local-first, on-device processing, no-automatic-upload and Professional-subscription wording without overflow. The verified live Building Analyst product-visuals page was left open for Wayne.
- Verification passed, so approved rollback deploy `6a5a39988351f4bd95f27bf9` was not restored. Final Netlify and GitHub readback reconfirmed deploy `6a5a61526773ce00086c4c1f` current and `origin/main` at `34a847f`. Wayne subsequently approved the remaining docs-only closeout: commit this tracker evidence and push only `codex/brand-contrast-system-20260716`, leaving `main` and production unchanged.
...
- Align `who-its-for.html` and `privacy.html` with the approved homepage/Building Analyst shell by reusing the zip-style icon wordmark, Home/Product/Pricing/About navigation, header email, solid blue "See What We Do" CTA, and zip-style footer.
- Preserve the existing Who It Is For and Privacy page content, privacy wording, cautious product maturity boundaries, no-form contact posture, and optional analytics wording.
- Add a scoped `.zip-secondary-page` CSS layer so reused zip shell components render correctly outside `body[data-page-type="preview"]`.
- Align the privacy notice intro to the top of the desktop card stack so the page no longer opens with a large empty left column.
- Bump the public `styles.css` cache key across the HTML pages that reference it so changed CSS is fetched after deployment.
- Update `scripts/rendered-release-smoke.mjs` so Who/Privacy rendered checks assert the new zip wordmark instead of the removed old `.brand-lockup-text small` strapline.
- Refresh `docs/codex/RELEASE_APPROVAL_PACKET.md` so Wayne's approval packet describes this current candidate instead of the historical BuildScan preview candidate.
- Refresh `docs/codex/FINAL_WEBSITE_APPROVAL_HANDOFF.md` and `docs/codex/GOAL_COMPLETION_AUDIT.md` so the current approval state is clear and the goal is not incorrectly marked complete while preview/production approval is still pending.
...
- Refreshed `npm run qa:release-staging-manifest` passed for the fifteen-file candidate inside the full local gate with artifact `output/release-staging-manifest/smoke-2026-07-08T07-18-26-105Z/release-staging-manifest-smoke.json`; modifiedTracked 15, untrackedCandidate 0, totalDirtyCandidate 15, stagingCommandPaths 15, approvalBoundaryChecked true.
- Refreshed full `npm run qa:release:local` passed for the fifteen-file cache-key candidate with artifact `output/release-local-gate/gate-2026-07-08T07-18-19-608Z/release-local-gate.json`; 37 steps. The gate confirmed public pages request `styles.css?v=20260708a`, release inventory dirtyCount 15 with zero secret findings and GLB external URI references 0, rendered screenshots `output/playwright/rendered-release-smoke-2026-07-08T07-19-45-967Z`, measurement evidence `output/measurement/evidence-2026-07-08T07-20-06-090Z`, Lighthouse performance 100, accessibility 100, best practices 100, SEO 100, CLS 0, LCP about 1.38s, and axe violations 0 across checked routes. The known warning-only residuals remain dev/release tooling dependency advisories and unavailable local Firefox/WebKit browser binaries; production dependency footprint remains 0 vulnerabilities.
- Final post-documentation `npm run qa:release-staging-manifest` passed with artifact `output/release-staging-manifest/smoke-2026-07-08T07-25-17-498Z/release-staging-manifest-smoke.json`; modifiedTracked 15, untrackedCandidate 0, totalDirtyCandidate 15, stagingCommandPaths 15, approvalBoundaryChecked true.
- Duplication audit after Wayne's request found no duplicate IDs, no duplicate headings, and no repeated long visible sentences across `index.html`, `building-analyst.html`, `who-its-for.html`, `privacy.html`, `404.html`, and `holding.html`. The audit still found softer copy repetition in the homepage and Building Analyst contact/boundary copy, which was tightened in `index.html` and `building-analyst.html`.
...
Status: completed for production; final source commit `8c595f6` is live as Netlify production deploy `6a4d6ccccf0a8038379c9abb`, the clean production gate passed, and live `/building-analyst` was verified as the new homepage-style Building Analyst route
...
- Commit `8c595f6` (`Harden deployed smoke diagnostics`) committed and pushed the cross-smoke release-control hardening to `codex/live-globe-loader-fix` and `main`.
- Netlify production deploy `6a4d6ccccf0a8038379c9abb` published final source commit `8c595f6` to `https://robsonai.co.uk`; deploy logs: `https://app.netlify.com/projects/robson-ai-website/deploys/6a4d6ccccf0a8038379c9abb`.
- Clean production release gate passed against `https://robsonai.co.uk` with artifact `output/release-production-gate/gate-2026-07-07T21-17-25-642Z/release-preview-gate.json`; 14 steps and dirtyCount 0.
- Live direct route verification on 2026-07-07 confirmed `/building-analyst`, `/building-analyst.html`, and cache-busted `/building-analyst?v=codex-live-check` all returned HTTP 200, included `building-analyst-zip-page`, included the new Building Analyst hero copy, and did not include the old lens UI.
- Fresh clean-browser render confirmed the live route title `Building Analyst | Professional assessment, evidence and reporting`, body class `zip-page building-analyst-zip-page`, H1 `Building Analyst turns evidence into review-ready intelligence.`, no old lens UI, and no console messages. Screenshot: `output/playwright/live-building-analyst-cache-check-2026-07-07T21-22/live-building-analyst.png`.

Done criteria:

- Full `npm run qa:release:local` passes. Completed.
- Building Analyst alignment commit is created and pushed. Completed.
- Netlify production deploy completes. Completed with final deploy `6a4d6ccccf0a8038379c9abb`.
- Production gate passes against `https://robsonai.co.uk`. Completed with clean-source artifact `output/release-production-gate/gate-2026-07-07T21-17-25-642Z/release-preview-gate.json`.
- Live `https://robsonai.co.uk/building-analyst` returns the new homepage-style Building Analyst page. Completed; cache-busted fresh-browser evidence captured.

Recommended next decision:

1. Recommended: Wayne opens `https://robsonai.co.uk/building-analyst?v=codex-live-check-20260707` for a final owner visual pass; if the old page appears, use `Cmd + Shift + R` because live production is already serving the new route.
2. Optional: install local Firefox/WebKit Playwright browsers later to remove the warning-only browser-coverage gap.
3. Optional: clean up the dev/release tooling dependency audit advisories in a separate non-release tranche.

## 2B. Completed Globe Loader Icon Prominence Tranche
...
- Increase the cropped Robson AI centre icon draw size by about 50%, from `size * 1.04` to `size * 1.56`.
- Keep the visible centre backplate removed.
- Keep the subtle canvas shadow only for contrast.
- Update public script references to `script.js?v=20260706a` so browsers fetch the changed drawing code.
- Preserve the Globe Loader globe, orbit animation, hero layout, product copy, BuildScan viewer, privacy baseline and release controls.
- Run focused local validation, then ask Wayne before any production deploy.

Out of scope:

- Globe Loader redesign, placement changes, orbit behaviour changes, CSS changes, copy changes, BuildScan asset changes, analytics/forms changes, privacy-policy changes, rollback, DNS/domain changes, customer-data handling, external messages, destructive git actions, or customer/system integrations.

Validation evidence:

- Wayne reviewed the live Globe Loader screenshot and asked for the centre icon to be about 50% bigger so it is more prominent within the globe circle.
- The immediately previous icon polish is live from commit `e1aab74` as Netlify production deploy `6a4b44f4473eb70007f8aace`.
...
- Local candidate updates the centre icon draw size in `script.js`, changes public page script references to `script.js?v=20260706a`, and adds that reviewed script source to the release-security allowlist.
...
## 2C. Completed Globe Loader Placement Hotfix
...
## 2D. Previous Reconciliation Tranche - Historical Context
...
- Create an isolated worktree from current `origin/main`.
- Merge the live-release branch `codex/docs-evidence-preservation-no-production-deploy` into the reconciliation branch.
- Resolve conflicts toward the currently published and production-gated Globe Loader/privacy release where conflicts affect website code, release scripts, and current release evidence.
- Preserve `main` history through Git history rather than keeping stale current-status text in live docs.
- Run local QA in the reconciliation worktree.
- Report the exact remote-action risk before Wayne chooses any push, PR, merge, or deploy step.
...
- Worktree created at `/private/tmp/robson-ai-website-main-reconciliation` on branch `codex/github-main-reconciliation-preflight` from `origin/main` commit `bd14eed`.
- `npm install` completed in the worktree; the known 17 moderate dev/release-tooling advisories remain and no `npm audit fix` was run.
- Baseline before merge: `git diff --check` passed and `npm run qa:release-inventory` passed with artifact `output/release-inventory/inventory-2026-07-05T15-18-23-724Z/release-candidate-inventory.json`, dirtyCount 0, zero secret findings and GLB externalUriCount 0.
- The first local merge attempt exposed conflicts in `index.html`, `styles.css`, `scripts/rendered-release-smoke.mjs`, `docs/codex/TRACKER.md`, `docs/codex/GOAL_COMPLETION_AUDIT.md`, `docs/codex/PUBLISH_READINESS_AUDIT.md`, `docs/codex/RELEASE_STAGING_MANIFEST.md`, `docs/codex/WEBSITE_APPROVAL_REVIEW_CHECKLIST.md`, `docs/codex/WEBSITE_RESTART_DESIGN_AUDIT.md`, and `docs/codex/WEBSITE_RESTART_PREVIEW_HANDOFF.md`.
- Conflict resolution policy: current product/release files resolve to the live production-gated branch because production is currently deploy `6a4a38a0eef7cd2aee692a52` from commit `db3922c`, with post-production evidence commit `9fcdf9f`.
- `git diff --check` and `git diff --cached --check` passed after resolving conflicts.
- The first full `npm run qa:release:local` attempt failed at `Netlify build` because the new worktree lacked ignored local `.netlify` link metadata. Existing ignored Netlify local state was copied from the original checkout without printing project IDs or staging it.
- The second full `npm run qa:release:local` attempt failed at `release staging manifest` because the manifest still described the earlier 30-file release scope. The manifest was updated to the reconciliation branch's 39 staged paths: 21 modified tracked files and 18 new candidate files.
- `npm run qa:release-staging-manifest` then passed with artifact `output/release-staging-manifest/smoke-2026-07-05T15-32-36-558Z/release-staging-manifest-smoke.json`; modifiedTracked 21, untrackedCandidate 18, totalDirtyCandidate 39, stagingCommandPaths 39.
- Final full `npm run qa:release:local` passed with artifact `output/release-local-gate/gate-2026-07-05T15-32-55-665Z/release-local-gate.json`; 37 steps.
- Release inventory inside the final gate passed with artifact `output/release-inventory/inventory-2026-07-05T15-33-02-461Z/release-candidate-inventory.json`; dirtyCount 39, zero secret findings and GLB externalUriCount 0.
- BuildScan viewer smoke passed with artifact `output/buildscan-viewer/smoke-2026-07-05T15-33-06-112Z`; direct and embedded model paths loaded successfully and the homepage requested `assets/globe-loader/world-countries-lite.json?v=20260705`.
- Product/design acceptance, responsive route and visual polish smokes passed with artifacts `output/product-design-acceptance/smoke-2026-07-05T15-33-28-898Z/product-design-acceptance-smoke.json`, `output/responsive-route/smoke-2026-07-05T15-33-37-628Z/responsive-route-smoke.json`, and `output/visual-polish/smoke-2026-07-05T15-34-02-406Z/visual-polish-smoke.json`.
- Rendered screenshots passed with artifact directory `output/playwright/rendered-release-smoke-2026-07-05T15-34-40-654Z`.
...
- Carry forward the local header icon and BuildScan demo-quality callout polish.
- Preserve the normalized supplied Globe Loader source folder inside this repo at `docs/codex/reference/globe-loader/`.
- Preserve Wayne's provided Downloads `Globe loader` folder and newer top-level `Globe Loader (standalone).html` file at `docs/codex/reference/globe-loader-imports/2026-07-05/`.
- Avoid shipping the prototype runtime as website code because the source version depends on React/D3/CDN assets and the standalone version is a large bundled prototype.
- Generate a compact public atlas asset at `assets/globe-loader/world-countries-lite.json` from the user-supplied standalone loader's embedded world-atlas topology.
- Add a production-safe local canvas Globe Loader treatment to the zip-style homepage hero using the existing Robson AI version 3 icon asset.
- Match Wayne's supplied screen recording more closely: pale globe, real country-detail geography, latitude/longitude grid, larger centred v3 icon and cleaner orbit arcs.
- Keep the Globe Loader separated from the hero field with an effectively invisible soft field on desktop/tablet; hide the decorative loader on phone-width layouts where it would compete with the hero copy.
- Respect `prefers-reduced-motion` and `qa-static` states.
- Fix release-inventory and staging-manifest parsing so intentional reference files with spaces in their names are handled safely.
- Update `privacy.html` to disclose the current/potential services a scanner can identify: Netlify hosting/security logs, optional Google Analytics 4 / Google tag script, browser local storage for consent, email-provider handling for mailto enquiries, self-hosted fonts/assets, and the same-origin BuildScan/Three.js model-viewer path.
- Update `docs/codex/RELEASE_STAGING_MANIFEST.md` with the exact candidate file set and explicit staging command.
...
- `curl -I http://127.0.0.1:8134/` returned HTTP 200.
- `node --check script.js` passed.
- `node --check scripts/release-candidate-inventory.mjs` passed.
- `node --check scripts/release-staging-manifest-smoke.mjs` passed.
- `git diff --check` passed.
- Direct Playwright Globe Loader check passed on `http://127.0.0.1:8134/`: the canvas was visible, nonblank, frame checksum changed after 700ms, and mobile horizontal overflow was false. Artifact: `output/globe-loader-check/2026-07-04T17-38-50-277Z/globe-loader-check.json`.
- Globe Loader screenshots were captured at `output/globe-loader-check/2026-07-04T17-38-50-277Z/desktop-globe-loader.png` and `output/globe-loader-check/2026-07-04T17-38-50-277Z/mobile-globe-loader.png`.
...
- Browser coverage advisory remains warning-only: Chromium passed; Firefox/WebKit Playwright binaries are unavailable locally. Artifact: `output/browser-coverage/smoke-2026-07-04T17-49-25-808Z/browser-coverage-smoke.json`.
- Dependency audit advisory remains warning-only: production vulnerabilities 0; dev/release tooling still has 17 moderate advisories. Artifact: `output/dependency-audit/summary-2026-07-04T17-48-08-759Z/dependency-audit-summary.json`.
- Final post-tracker `npm run qa:release-inventory` passed with artifact `output/release-inventory/inventory-2026-07-04T17-53-39-669Z/release-candidate-inventory.json`; dirtyCount 19, zero secret findings, GLB externalUriCount 0.
- Final post-tracker `npm run qa:release-staging-manifest` passed with artifact `output/release-staging-manifest/smoke-2026-07-04T17-53-40-725Z/release-staging-manifest-smoke.json`; 11 modified tracked files, 8 untracked candidate files, 19 staging paths.
- Wayne supplied `Screen Recording 2026-07-04 at 18.51.45.mov` as the target Globe Loader look. Quick Look reference thumbnail generated at `output/globe-loader-video-compare/Screen Recording 2026-07-04 at 18.51.45.mov.png`.
...
- Wayne later asked Codex to move the Downloads `Globe loader` folder and `Globe Loader (standalone).html` file into the project. They were moved to `docs/codex/reference/globe-loader-imports/2026-07-05/`, leaving no matching top-level Globe Loader items in `/Users/wayne/Downloads`.
- Reference-match Playwright check passed after the latest visual update: pale/bright pixels, dark outline pixels, nonblank canvas, animation checksum change, and no mobile overflow. Artifact: `output/globe-loader-reference-match/2026-07-05T08-17-37-259Z/globe-loader-reference-match.json`; screenshots at `desktop-reference-match.png` and `mobile-reference-match.png`.
- Focused QA after the latest visual update passed:
...
- After the Downloads move, `node --check scripts/release-candidate-inventory.mjs` passed, `git diff --check` passed, and `npm run qa:release-inventory` passed with artifact `output/release-inventory/inventory-2026-07-05T08-50-12-991Z/release-candidate-inventory.json`; dirtyCount 28, zero secret findings, GLB externalUriCount 0.
- SHA-256 preservation notes after whitespace normalisation: reference/folder standalone HTML `2d79420cf7c65804f99d4182be4da188f8440aca4ea9a7f8785dff0557a64616`; moved newer top-level standalone HTML `58e3c42a9d6e7b0fda3b416b3b567aca2eedfe937e31125c46db97cc55e12ab2`.
...
- Atlas-detail Globe Loader browser check passed on `http://127.0.0.1:8134/`: `assets/globe-loader/world-countries-lite.json?v=20260705` loaded with HTTP 200, the canvas was visible and nonblank, dark-detail pixels were present, the decorative loader was hidden on 390px mobile, no horizontal overflow was detected, and no console/page errors were recorded. Artifact and screenshots: `output/globe-loader-atlas-match/2026-07-05T09-48-16-437Z/`.
...
- Live production privacy triage after the iubenda email found no active GA4 Measurement ID and no external runtime requests on first load or after the accept path. This suggests the iubenda report is likely flagging prepared/allowed services or stale scan results rather than active analytics execution.
- Privacy service disclosure baseline updated `privacy.html` locally to disclose Netlify hosting/security logs, optional GA4/Google tag script, localStorage consent state, mailto/email-provider handling, self-hosted fonts/assets, and same-origin BuildScan/Three.js model-viewer assets.
...
Status: completed and production-gated; Netlify production deploy `6a4933ec2451857b37ea20b4` is live on `https://robsonai.co.uk`
...
Out of scope after the approved production publish:

- Rollback, branch push, GitHub PR, DNS/domain changes, analytics/forms/customer-data capture, new pricing/commercial promises, Apple/platform implementation, external messages, destructive git actions, or further committing/pushing without Wayne's later approval.
...
- Wayne approved option `1` on 2026-07-04 17:11 BST for explicit-path staging, local commit, Netlify non-production preview deploy and deployed preview-gate validation; this approval was executed.
- Wayne approved option `1` again on 2026-07-04 for the production deploy from commit `f2df3c1` and production release-gate validation; this approval was executed.
- Rollback, branch push, GitHub PR, DNS/domain changes, analytics/forms/customer data, further production deploys and external messages still require explicit later approval.
...
- Staged full local release gate passed before commit: `output/release-local-gate/gate-2026-07-04T16-12-44-423Z/release-local-gate.json`; result pass, 37 steps.
- Local commit created: `f2df3c1` (`Restore zip-faithful website motion preview`).
- Netlify non-production preview deploy succeeded from a clean `git archive` of commit `f2df3c1`: deploy `6a4931e3a196083c018fa0bb`, preview URL `https://zip-faithful-motion--robson-ai-website.netlify.app`, logs `https://app.netlify.com/projects/robson-ai-website/deploys/6a4931e3a196083c018fa0bb`.
- Deployed preview gate passed: `QA_BASE_URL="https://zip-faithful-motion--robson-ai-website.netlify.app" npm run qa:release:preview`; artifact `output/release-preview-gate/gate-2026-07-04T16-16-54-785Z/release-preview-gate.json`; result pass, 14 steps.
- Preview gate evidence includes clean release inventory `output/release-inventory/inventory-2026-07-04T16-16-54-926Z/release-candidate-inventory.json` with dirtyCount 0, secret findings 0, GLB externalUriCount 0; deployed headers/source-deny `output/release-headers/smoke-2026-07-04T16-17-02-455Z/release-header-smoke.json`; deployed BuildScan viewer `output/buildscan-viewer/smoke-2026-07-04T16-17-02-806Z`; keyboard `output/playwright/keyboard-release-smoke-2026-07-04T16-17-12-266Z`; semantic SEO `output/semantic-seo/smoke-2026-07-04T16-17-18-927Z/semantic-seo-smoke.json`; product/design `output/product-design-acceptance/smoke-2026-07-04T16-17-32-222Z/product-design-acceptance-smoke.json`; responsive `output/responsive-route/smoke-2026-07-04T16-17-40-993Z/responsive-route-smoke.json`; visual polish `output/visual-polish/smoke-2026-07-04T16-18-12-433Z/visual-polish-smoke.json`; browser coverage advisory `output/browser-coverage/smoke-2026-07-04T16-19-06-781Z/browser-coverage-smoke.json`; rendered screenshots `output/playwright/rendered-release-smoke-2026-07-04T16-19-15-017Z`; measurement smoke `output/measurement/smoke-2026-07-04T16-19-27-393Z`.
- Netlify production deploy succeeded from a clean `git archive` of commit `f2df3c1`: deploy `6a4933ec2451857b37ea20b4`, production URL `https://robsonai.co.uk`, unique deploy URL `https://6a4933ec2451857b37ea20b4--robson-ai-website.netlify.app`, logs `https://app.netlify.com/projects/robson-ai-website/deploys/6a4933ec2451857b37ea20b4`.
- Production release gate passed: `QA_PRODUCTION_URL="https://robsonai.co.uk" CONFIRM_PRODUCTION_VERIFICATION=true npm run qa:release:production`; artifact `output/release-production-gate/gate-2026-07-04T16-25-32-131Z/release-preview-gate.json`; result pass, 14 steps.
- Production gate evidence includes release inventory `output/release-inventory/inventory-2026-07-04T16-25-32-251Z/release-candidate-inventory.json` with docs-only dirtyCount 4 at gate start, secret findings 0, GLB externalUriCount 0; dependency advisory `output/dependency-audit/summary-2026-07-04T16-25-32-476Z/dependency-audit-summary.json`; release security `output/release-security/smoke-2026-07-04T16-25-36-293Z/release-security-smoke.json`; deployed headers/source-deny `output/release-headers/smoke-2026-07-04T16-25-39-683Z/release-header-smoke.json`; deployed BuildScan viewer `output/buildscan-viewer/smoke-2026-07-04T16-25-39-989Z`; keyboard `output/playwright/keyboard-release-smoke-2026-07-04T16-25-45-967Z`; semantic SEO `output/semantic-seo/smoke-2026-07-04T16-25-50-723Z/semantic-seo-smoke.json`; product/design `output/product-design-acceptance/smoke-2026-07-04T16-25-58-674Z/product-design-acceptance-smoke.json`; responsive `output/responsive-route/smoke-2026-07-04T16-26-04-668Z/responsive-route-smoke.json`; visual polish `output/visual-polish/smoke-2026-07-04T16-26-23-666Z/visual-polish-smoke.json`; browser coverage advisory `output/browser-coverage/smoke-2026-07-04T16-26-42-647Z/browser-coverage-smoke.json`; rendered screenshots `output/playwright/rendered-release-smoke-2026-07-04T16-26-49-860Z`; measurement smoke `output/measurement/smoke-2026-07-04T16-27-00-373Z`.
- Post-production docs hygiene passed: stale-current-status search found no active contradictions; `git diff --check` passed; `npm run qa:release-inventory` passed with `output/release-inventory/inventory-2026-07-04T16-35-32-032Z/release-candidate-inventory.json`, dirtyCount 6, secret findings 0, GLB externalUriCount 0.
- Publish-readiness estimate for the full process to production: 100%. Local implementation, local QA, local commit, Netlify preview deploy, preview gate, production deploy and production release gate are complete.
- Recommended next decision: `1` approve a docs/source-control alignment tranche so the production evidence docs can be committed and the branch/main state can be reconciled without accidentally triggering an unmanaged deploy.

## 3. Superseded Active Tranche Record

Tranche name: `website-quality-restart-from-original-goal`
Status: superseded by Wayne visual rejection on 2026-07-04; Netlify production deploy `6a48c6e1a19608e3698fa160` was replaced by zip-faithful deploy `6a4933ec2451857b37ea20b4`
...
- Adapt Wayne's redesign zip into the current static website without copying prototype runtime dependencies or unsupported claims.
- Add the BuildScan model-view and opt-in GLB viewer elements into the zip-inspired design.
...
- A focused zip-adapted design tranche is implemented locally without publishing.
